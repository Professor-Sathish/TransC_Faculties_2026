...
class CBankInterface:
    ...
    def __init__(self, c_program_path="./i7"):
        ...
        # c_program_path="i7"          # Without ./
        # c_program_path="./i7"        # With ./
        # c_program_path=os.path.abspath("i7")  # Absolute path
            ...
            if os.path.exists(self.data_file):
                with open(self.data_file, "rb") as f:
                    for i in range(100):
                        ...
                        if len(data) == 40:
                            try:
                                acct_num, last_name_bytes, first_name_bytes, balance = struct.unpack(
                                    "I15s10sd", data)

                                # V6 validation: valid account number and reasonable balance
                                if (acct_num != 0 and acct_num >= 1
                                        and acct_num <= 100
                                        and balance >= -1000000.0
                                        and balance <= 10000000.0):

                                    last_name = last_name_bytes.decode(
                                        'utf-8', errors='ignore').rstrip(
                                            '\x00').strip()
                                    first_name = first_name_bytes.decode(
                                        'utf-8', errors='ignore').rstrip(
                                            '\x00').strip()

                                    # Clean non-printable characters (V6 sanitization)
                                    last_name = ''.join(c for c in last_name
                                                        if c.isprintable())
                                    first_name = ''.join(c for c in first_name
                                                         if c.isprintable())
        ...
        if result['success']:
            ...
            if "already contains information" in result['output']:
                return jsonify({
                    ...
                    False,
                    "message":
                    f"Account #{account_num} already exists"
                })
            else:
                return jsonify({
                    "success": False,
        ...
        if result['success']:
            ...
            if "has no information" in result['output']:
                return jsonify({
                    ...
                    False,
                    "message":
                    f"Account #{account_num} does not exist"
                })
            else:
                return jsonify({
                    "success": False,
...
def export_accounts():
    ...
    try:
        ...
        if result['success']:
            return jsonify({
                "success": True,
                "message": "Accounts exported to accounts.txt"
            })
        else:
            return jsonify({
                "success": False,
    ...
    try:
        ...
        if result['success']:
            ...
            if criterion == 3 or criterion == 4:
                            ...
                            try:
                                ...
                                if 1 <= acct_num <= 100:  # Valid account number
                                    ...
                                    return jsonify({
                                        "success": True,
                                        "message":
                                        f"{'Maximum' if criterion == 3 else 'Minimum'} balance account found",
                                        "account": {
                                            'acct_num': acct_num,
                                            'last_name': last_name,
                                            'first_name': first_name,
                ...
                return jsonify({
                    "success": False,
                    "message": "No valid account found for min/max"
                })
                ...
                if sorted_accounts:
                    ...
                    if accounts:
                        ...
                        return jsonify({
                            "success": True,
                            "message":
                            f"Sorted by {'Balance' if criterion == 1 else 'Name'} ({'Desc' if order == 2 else 'Asc'}) - Python fallback",
                            "accounts": sorted_accounts
                        })

                    return jsonify({
                        "success": False,
                        "message": "No accounts to sort"
                    })

        else:
            return jsonify({
                "success": False,
                "message": f"Sort failed: {result['error']}"
            })
...
if __name__ == '__main__':
    ...
    print(
        "🚀 V6 Features: Advanced Sorting, Data Validation, Corruption Filtering"
    )
